...
class GaussCRF(nn.Module):
    ...
    def __init__(self, conf, shape, nclasses=None, use_gpu=True):
        ...
        if conf["col_feats"]["use_bias"]:
            register(
                "col_sdims", torch.Tensor([1 / conf["col_feats"]["sdims"]])
            )
        else:
            self.col_sdims = None
    ...
    def forward(self, unary, img, num_iter=5):
        ...
        self.CRF.add_pairwise_energies(
            [pos_feats, col_feats], compats, conf["merge"]
        )
    ...
    def _create_mesh(self, requires_grad=False):
        ...
        mesh = np.array(
            np.meshgrid(*hcord_range, indexing="ij"), dtype=np.float32
        )
    ...
    def _create_convolutional_filters(self, features):
        ...
        for dx in range(-span, span + 1):
            for dy in range(-span, span + 1):
                ...
                feat_t = features[
                    :, :, dx1 : _negative(dx2), dy1 : _negative(dy2)
                ]
                feat_t2 = features[
